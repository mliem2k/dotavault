...
function PlayButton({ compact }: { compact?: boolean }) {
  return (
    <a
      ...
      className={`text-white uppercase transition-colors hover:border-white ${compact ? 'text-[13px] tracking-[1px]' : 'shrink-0 text-[13px] sm:text-[18px] tracking-[1px] sm:tracking-[3px]'} font-display`}
      style={{
...
function GitHubIcon() {
  return (
    <a
      ...
      className="flex h-11 w-11 shrink-0 items-center justify-center text-white transition-colors hover:text-gold"
    >
  ...
  return (
    <nav className="relative z-50">
      <div className="mx-auto flex h-20 w-full max-w-[1800px] items-center px-4 sm:px-6 gap-4 sm:gap-8">
        <Link
          to="/"
          className="text-lg sm:text-2xl font-bold tracking-wide text-foreground shrink-0 font-display"
        >
          dotavault
        </Link>
        ...
        <div
          className="hidden md:flex items-center gap-8 uppercase text-white text-[18px] tracking-[3px] flex-1 font-display"
          style={{ fontWeight: 600 }}
        >
          {NAV_LINKS.map((l) => (
            <Link key={l.to} to={l.to} className="text-white transition-colors hover:text-gold">
              {l.label}
            </Link>
          ))}
        </div>
        ...
        >
          {NAV_LINKS.map((l) => (
            <Link
              ...
              className="py-3 uppercase text-white text-[16px] tracking-[2px] transition-colors hover:text-gold font-display"
              style={{ fontWeight: 600, borderBottom: '1px solid rgba(255,255,255,0.06)' }}
            >
              {l.label}
            </Link>
          ))}
