...
export function TopHeroesThisPatchTable({
  playerHeroes,
  heroStats,
}: {
  ...
  // Win% leads by default here (unlike the lifetime Most Played Heroes
  // table, which defaults to Games): this table answers "what's working
  // this patch," not "what have I played most."
  const { key: sortKey, dir: sortDir, onSort } = useSort<SortKey>('winrate', 'desc')
  const [showAll, setShowAll] = useState(false)

  const qualifying = playerHeroes.filter((h) => h.games >= MIN_GAMES)
  ...
  if (qualifying.length === 0) {
    return (
      <p className="text-sm text-muted py-4 text-center">
        No heroes with {MIN_GAMES}+ games played this patch yet.
      </p>
    )
  }
