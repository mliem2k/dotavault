...
const COMPONENTS: Component[] = [
  ...
  { label: 'Deaths', key: 'deaths', value: (p) => p.deaths, points: (p) => 3 - p.deaths * 0.3 },
  {
    label: 'CS',
    key: 'cs',
    value: (p) => p.last_hits + p.denies,
    points: (p) => (p.last_hits + p.denies) * 0.003,
  },
  { label: 'GPM', key: 'gpm', value: (p) => p.gold_per_min, points: (p) => p.gold_per_min * 0.002 },
  {
    label: 'Towers',
    key: 'towers',
    value: (p) => p.towers_killed ?? 0,
    points: (p) => (p.towers_killed ?? 0) * 1,
  },
  {
    label: 'Roshan',
    key: 'roshan',
    value: (p) => p.roshans_killed ?? 0,
    points: (p) => (p.roshans_killed ?? 0) * 1,
  },
  {
    label: 'Teamfight',
    key: 'teamfight',
  ...
  },
  {
    label: 'Obs Wards',
    key: 'obswards',
    value: (p) => p.obs_placed ?? 0,
    points: (p) => (p.obs_placed ?? 0) * 0.5,
  },
  {
    label: 'Stacks',
    key: 'stacks',
    value: (p) => p.camps_stacked ?? 0,
    points: (p) => (p.camps_stacked ?? 0) * 0.5,
  },
  {
    label: 'Runes',
    key: 'runes',
    value: (p) => p.rune_pickups ?? 0,
    points: (p) => (p.rune_pickups ?? 0) * 0.25,
  },
  {
    label: 'First Blood',
    key: 'firstblood',
  ...
  },
  {
    label: 'Stuns',
    key: 'stuns',
    value: (p) => Math.round(p.stuns ?? 0),
    points: (p) => (p.stuns ?? 0) * 0.05,
  },
]
  ...
  return (
    <div className="overflow-x-auto font-dota" style={{ background: C.panel }}>
      ...
      <Table className="w-full" style={{ minWidth: 1100 }}>
        <TableHeader>
          <TableRow
            className="hover:bg-transparent text-[12px] uppercase text-slate-muted border-none"
      ...
      <p className="px-4 py-3 text-[12px] text-slate-muted">
        Standard fantasy scoring: 0.3/kill, 3 minus 0.3/death, 0.003/CS, 0.002/GPM, 1/tower,
        1/Roshan, 3 x teamfight share, 0.5/observer, 0.5/stack, 0.25/rune, 4/first blood, 0.05/stun
        second. Small values show points over raw stat.
      </p>
    </div>
  )
}
