  ...
  return (
    <div style={{ background: C.panel }}>
      ...
      <div className="overflow-x-auto p-3">
        <table style={{ borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th />
              {all.map((v) => (
                <th key={v.player_slot} style={{ padding: 3 }}>
            ...
            {all.map((p) => {
              ...
              return (
                <tr key={p.player_slot}>
                  <td style={{ padding: 3, paddingRight: 8 }}>
                    <div className="flex items-center gap-1.5">
                      <span style={{ width: 3, height: 30, background: isRadiant ? C.green : C.red, display: 'inline-block' }} />
        ...
        <div className="mt-2.5 text-[13px]" style={{ color: C.dim, fontFamily: 'var(--font-dota)' }}>
          Rows kill columns.
        </div>
      </div>
    </div>
      ...
      <div className="p-4 grid gap-x-10 gap-y-2" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(270px, 1fr))' }}>
        {events.map((e, i) => (
          ...
          <div key={i} className="flex items-center gap-2">
            <span className="w-14 text-right text-[14px] tabular-nums shrink-0" style={{ color: C.dim, fontFamily: 'var(--font-dota)' }}>
              {fmtClock(e.time)}
            </span>
            <span style={{ width: 3, height: 24, background: e.radiant ? C.green : C.red }} />
            <HeroIcon hero={e.killer} size={30} />
            <span className="text-[13px]" style={{ color: C.dim }}>killed</span>
          ...
          return (
            ...
            >
              <div className="w-32 shrink-0" style={{ fontFamily: 'var(--font-dota)' }}>
                ...
                <div className="text-[12px] uppercase" style={{ color: C.dim, letterSpacing: '1px' }}>
                  {f.deaths} deaths
                </div>
              </div>
              ...
              <div className="w-48 shrink-0 text-right" style={{ fontFamily: 'var(--font-dota)' }}>
                <div className="text-[16px] tabular-nums" style={{ color: winner === 'radiant' ? C.green : C.red }}>
